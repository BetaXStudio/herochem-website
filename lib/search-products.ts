...
import { products, type Product, type CategoryLabel, type Brand } from './products-database';
...
export const searchProductsDatabase = (query: string, limit: number = 10): SearchProduct[] => {
  ...
  const scoredResults = products.map(product => {
    ...
    if (product.filterType && product.filterType.toLowerCase().includes(searchTerm)) {
      score += 5;
    }
