...
export default function CheckoutModal({ isOpen, onClose, onAuthRequired, cartItems }: CheckoutModalProps) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      return () => {
        const savedScrollY = parseInt(document.body.dataset.scrollPosition || '0');
  ...
  const handleAddAddress = async () => {
    ...
    // Validate form
    if (!addressForm.full_name.trim() || !addressForm.street.trim() || 
        !addressForm.house_number.trim() || !addressForm.city.trim() || 
  ...
  const calculateShipping = () => {
    return shippingMethod === 'standard' ? 19.00 : 0;
  };
  ...
  const calculateTotal = () => {
    return calculateSubtotal() + calculateShipping() + calculatePaymentFee();
  };
  ...
  return (
    <>
      ...
      `}</style>
      <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[9999] flex items-start justify-center pt-16 p-4" style={{ animation: 'backdropFadeIn 0.3s ease-out' }}>
        <div 
              ...
              <div className="space-y-6">
                ...
                <div className="bg-neutral-800 rounded-lg p-4">
                  ...
                  <div className="space-y-3">
                    {cartItems.map((item) => (
                      <div key={`${item.handle}-${item.variant || 'default'}`} className="flex items-center gap-3">
                        {item.image && (
                          <img
                            src={item.image}
                            alt={item.title}
                        ...
                        <div className="flex-1">
                          ...
                          {item.variant && (
                            <p className="text-neutral-400 text-xs">{item.variant}</p>
                          )}
                          <div className="flex items-center justify-between">
                            <span className="text-neutral-300 text-sm">Qty: {item.quantity}</span>
                ...
                <div className="bg-neutral-800 rounded-lg p-4">
                            ...
                            <div className="flex items-start justify-between">
                              <div>
                                <div className="flex items-center gap-2">
                                  <h4 className="text-white font-medium">{address.full_name}</h4>
                                  {address.is_primary && (
                  ...
                  {showAddAddressForm && (
                    <div className="mt-4 p-4 bg-neutral-700 rounded-lg">
                      <h4 className="text-white font-medium mb-3">Add New Address</h4>
                      <div className="space-y-3">
                        <input
                          type="text"
                        ...
                        <div className="flex items-center">
                          ...
                          />
                          <label htmlFor="is_primary_checkout" className="ml-2 text-sm text-neutral-300">
                    ...
                    <div className="p-3 border border-[#e91111] bg-[#e91111]/10 rounded-lg">
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-neutral-300 text-sm">Delivery within 5-7 business days</p>
                        </div>
                        <div className="text-right">
                          <span className="text-white font-medium">{formatPrice(19.00)}</span>
                        </div>
                      </div>
                    </div>
                    <p className="text-neutral-400 text-xs">More shipping options will be available soon</p>
                  </div>
                </div>
                  ...
                  <div className="space-y-2">
                    ...
                    >
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-neutral-300 text-sm">No additional fee</span>
                      </div>
                    </div>
                    ...
                    >
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-green-400 text-sm">-5% discount</span>
                      </div>
                    </div>
                    ...
                    >
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-orange-400 text-sm">+5% fee</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-neutral-800 rounded-lg p-4">
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="text-white">{formatPrice(calculateShipping())}</span>
                    </div>
                    {calculatePaymentFee() !== 0 && (
                      <div className="flex justify-between">
                        <span className="text-neutral-400">
                          Payment {calculatePaymentFee() > 0 ? 'Fee' : 'Discount'}:
                        </span>
                        <span className={calculatePaymentFee() > 0 ? 'text-orange-400' : 'text-green-400'}>
                          {calculatePaymentFee() > 0 ? '+' : ''}{formatPrice(calculatePaymentFee())}
                        </span>
                      </div>
                    )}
                    <div className="border-t border-neutral-600 pt-2 mt-2">
                      <div className="flex justify-between font-semibold">
                        ...
                        <span className="text-white text-lg">{formatPrice(calculateTotal())}</span>
                      </div>
                    </div>
                  </div>
                </div>
